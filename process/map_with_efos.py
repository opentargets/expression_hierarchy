...
def filter_parents(parents, parent_type, minimal_tissues):
  ...
  filtered_parents = {}
  for (parent_name, parent_info) in parents.iteritems():
    filtered_tissues = [tissue_name
                        for tissue_name in parent_info["children"]
                        if tissue_name in minimal_tissues]
    if len(filtered_tissues) > 0:
      filtered_parents[parent_name] = {
        "label": parent_info["label"],
        "efo_code": parent_info["efo_code"],
        "children": filtered_tissues
      }
  # filtered_parents = {parent_name: [tissue_name 
  #                                   for tissue_name in parent
  #                                   if tissue_name in minimal_tissues]
  #                     for (parent_name, parent_info) in parents.iteritems()}
